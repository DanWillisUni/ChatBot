...
class NeuralNetwork:
    ...
    def forwardPropagation(self, inputVectors):
        ...
        for i in range(1, layers + 1):#for each layer except the input
            if i == 1:#if it is the first layer after input
                values['Z' + str(i)] = np.dot(self.params['W' + str(i)], inputVectors) + self.params['B' + str(i)]#use the input to calculate Z
                values['A' + str(i)] = relu(values['Z' + str(i)])#set the value of the A for the layer to the Relu value of the Z for the layer
            else:
                values['Z' + str(i)] = np.dot(self.params['W' + str(i)], values['A' + str(i - 1)]) + self.params['B' + str(i)]#use the previuos layers A to calculate Z
    ...
    def update(self, gradients, learningRate):
        ...
        for i in range(1, layers + 1):#for each layer
            newParams['W' + str(i)] = self.params['W' + str(i)] - learningRate * gradients['W' + str(i)]#set the new weight params to the old params - gradient
            newParams['B' + str(i)] = self.params['B' + str(i)] - learningRate * gradients['B' + str(i)]#set the new bias params to the old params - gradient
    ...
    def train(self, inputVectors, targets, iterations):
        ...
        for currentIteration in range(iterations):#for each iteration
            ...
            gradient = self.backwardPropagation(values, inputVectors.T, targets.T)#get the gradients from the backwardsPropagation
            self.update(gradient, 0.1)#update the params
            ...
            if currentIteration % 1000 == 0:#if the current iteration is divisable by 1000
                ...
                for dataIndex in range(len(inputVectors)):
                    ...
                    squareError = (prediction - target) ** 2#square the difference between the target and the prediction
                    sumSquaredError = sumSquaredError + squareError#add the squared difference to the sum
                rootMeanSquareErrors.append(math.sqrt(sumSquaredError/len(targets)))#add the root mean squared value of all te errors to the array
    ...
    def startTraining(self,maxDataSize,iterations):
        ...
        #plot the graph
        for i in range(0,len(trainingError),100):#this splits the graph up into one hundred thousand iterations per graph
            #plotting graph
    ...
    def predictNice(self,delay,nameA,nameB):
        ...
        input = np.array([[delay, int(comparingStations[getStationCompareIndex(stationDic, nameA, nameB)])]])#format the input to the NN
...
def getStationCompareIndex(stationDic, A, B):
    ...
    '''
    return (stationDic[A] * len(stationDic)) + stationDic[B] #multiplys the id of the first station by the length of the array (77) then ads the id of the second station
    ...
    if exists(appSettings.getStationComparePath()):#if the textfile exists
        ...
        for a in range(len(allStations)):#for each station
            ...
            for b in range(a + 1, len(allStations)):#for each station that hasnt been done
                ...
                normalDistance = sph.compareStations(appSettings.getConnStr(), nameA, nameB)#get the normal time between station A and B
                if normalDistance != 'None':#if not Null
                    comparingStations[getStationCompareIndex(stationDic,nameA, nameB)] = int(normalDistance)#set the index to normal distance
                    comparingStations[getStationCompareIndex(stationDic,nameB, nameA)] = 0 - int(normalDistance)#set the mirrored index to the negative of the normal distance
...
def getNNData(maxDataSize,removeOutliers):
    ...
    for rid in rids:#for each RID
        ...
        for a in range(len(ridData)):#for all the data on the rid
            ...
            for b in range(a + 1,len(ridData)):#for all the data after station A
                ...
                input = np.array([delayA, int(comparingStations[getStationCompareIndex(stationDic,nameA, nameB)])])#create the input array for the NN
                ...
                testForOutliers.append(abs(delayA - delayB))#add the difference between delay to station A and delay to station B into the array
    if removeOutliers:  # if the outliers are to be removed
        ...
        for i in range(len(targets)):  # for all the indexes
            if abs(inputs[i * 2] - targets[i]) > maxAllowedDifference:#if the difference between delay A and delay B is causes it to be an outlier
        ...
        for indexToRemove in indexsToRemove:#for all the indexes
            ...
            inputs = np.delete(inputs, ((indexToRemove - count) * 2) + 1)#delete the normal station time for the index
